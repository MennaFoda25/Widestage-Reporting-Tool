...
usersSchema.statics.createTheUser = async function (req, userData, done) {
    ...
    try {
        const existingUser = await User.findOne({ "userName": userData.userName, companyID: req.user.companyID });
        if (existingUser) {
...
usersSchema.statics.setViewedContextHelp = async function (req, done) {
    ...
    try {
        ...
        if (!findUser) {
            return done({ result: 0, msg: "No user found, can't update the user context help" });
        }
        ...
        await Users.updateOne({ "_id": userID }, { $set: { "contextHelp": findUser.contextHelp } });
...
usersSchema.statics.setStatus = async function (req, done) {
    ...
    try {
        const findUser = await this.findOne({ "_id": userID, "companyID": req.user.companyID });
        if (!findUser) {
            return done({ result: 0, msg: "No user found for this company, can't update the user status" });
        }
...
usersSchema.statics.isValidUserPassword = async function (username, password, done) {
    try {
        const user = await this.findOne({ $or: [{ 'userName': username }, { 'email': username }], status: 'active' });
        if (!user) return done(null, false, { message: `User ${username} does not exist or is inactive` });
...
usersSchema.statics.rememberPassword = async function (email, url, done) {
    ...
    const hash_change_password = crypto.createHash('md5').update(email).digest('hex');

    const postData = {
        id: "52d66ea2c6b91ae01f00000a",
        email: email,
        tags: JSON.stringify({ CHANGEPWDURL: `${url}login/#/change-password/${hash_change_password}` })
    };

    try {
        await this.updateOne({ "email": email }, { $set: { "hash_change_password": hash_change_password } });
...
usersSchema.statics.changePassword = async function (req, done) {
    ...
    try {
        ...
        await User.updateOne({ "_id": user._id }, { $set: { "salt": salt, "hash": hash, "hash_change_password": null } });

        const Configurations = connection.model('Configurations');
        const configuration = await Configurations.getConfiguration('log-user-pwd-change');
