...
exports.create= (model)=>{
  asyncHandler(async (req,res) => {
    ...
    return res.status(201).send({ result: 1, msg: "Item created", item: item.toObject() });
  })
    // Handle any errors that may occur
    return next(new ApiError(err.message, 500)); // Use a custom ApiError to encapsulate errors
