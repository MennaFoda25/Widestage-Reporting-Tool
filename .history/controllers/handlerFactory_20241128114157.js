...
Controller.method('update', async function (req, done) {
    ...
    try {
        ...
        if (numAffected > 0) {
            done({ result: 1, msg: `${numAffected} record(s) updated.` });
        } else {
            done({ result: 0, msg: "Error updating record, no records have been updated." });
        }
    } catch (err) {
