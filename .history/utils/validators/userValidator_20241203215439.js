...
exports.createUserValidator = [
  ...
  check("password")
    ...
    .isLength({ min: 3 })
    .withMessage("Too short").custom((password,{req})=>{
        if(password !== req.passwordConfirm){
            throw new Error('Password Confirmation is ')
        }

    }),

    check('passwordConfirm').notEmpty().withMessage('password confirmation needed'),
